...
#### API-access
More customized extratcs from NMDbiotic can be made programatically by using the APIs (https://confluence.imr.no/display/API/API+Documentation). The Biotic API provides various services for downloading biotic-data sets, and the cruise API allows lookup of relations between biotic data-sets and cruises. There is currently no API for cruise-timeseries, but a semi-automatic approach can be realized by downloading the cruiseseries table and reading in that.
...
* Use the tools in datasetexplorer to fetch all data for each year of interest. Manual download by year, and then subsequent filtering in R.
...
Not all levels are mandatory, but records cannot be recorded without the parent record. For instance, an 'individual' is always linke to a 'catchsample', but not all 'catchsample' records have any individual records. A record is identified with by the fields listed as 'keys' in the format documentation, along its parent record. When read with RstoxData these records are organized in data.tables corresponding to each level, and keys from all records higher in the hierarchy is included as well.

Analysts often prefer the records they need into one big table. Since the hierarchy branches off below individual, it is not possible to construct one general table for analysis. Also note that key parameters like age- and tag- parameters are in a many to one relationship with individuals, so it is often necessary to do some selective filtering of lower levels if these are to be used in a table with one individual for each row.
...
We will also define a function that pulls data for a set of paths and compiles them into one xml file:
...
In some cases it is desirable to filter data before converting it to one 2-dimensional tables. For instance, you may want to remove species that are not of interst before conversion, in order to reduce the number of warnings that needs to be checked. RstoxData provides functions for specifying these kinds of filters for all the hierarchical data formats it supports. See for instance RstoxData::FilterBiotic or RstoxData::FilterStoxBiotic.
...
```
We have retained only catchsamples with cod:
