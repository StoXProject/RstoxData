...
### 	StoxAcousticData$Log[LogOrigin == "middle", StartDateTime := DateTime - LogDuration / 2]
### 	StoxAcousticData$Log[LogOrigin == "end", StartDateTime := DateTime - LogDuration]
### 	
...
#' @export
#' 
StoxDataStartMiddleStopDateTime <- function(StoxData, type = c("Acoustic", "Biotic")) {
	type <- match.arg(type)
	
	if(type == "Acoustic") {
		# Fill the start, middle and end DateTime with the DateTime directly, given the LogOrigin:
		# Fill the StartDateTime with the DateTime directly, given the LogOrigin:
		StoxData$Log[LogOrigin == "start", StartDateTime := DateTime]
		# Interpret StartDateTime from LogDuration and LogOrigin:
		StoxData$Log[LogOrigin == "middle", StartDateTime := DateTime - LogDuration / 2]
		StoxData$Log[LogOrigin == "end", StartDateTime := DateTime - LogDuration]
		
		# Extrapolate to the middle and end times:
		StoxData$Log[, MiddleDateTime := StartDateTime + LogDuration / 2]
		StoxData$Log[, StopDateTime := StartDateTime + LogDuration]
	}
	else if(type == "Biotic") {
		...
		StoxData$Station[, StartDateTime := DateTime]
		StoxData$Station[, MiddleDateTime := DateTime]
		StoxData$Station[, StopDateTime := DateTime]
	}
	else {
