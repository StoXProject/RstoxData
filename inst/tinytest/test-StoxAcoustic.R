...
sa1 <- RstoxData:::StoxAcoustic(list(defaultParseEchosounder))
...
sa2 <- RstoxData:::StoxAcoustic(list(streamParseEchosounder))
...
for(item in icesFiles) {
    ...
    icesDataA <- RstoxData:::StoxAcoustic(list(RstoxData:::readXmlFile(paste0(exampleDir, "/", item), stream = F)))
	expect_equal(nrow(icesDataA$Log), 2)

	#context(paste("test-StoxAcoustic: ICES acoustic data", item, "to StoxAcoustic: Stream"))
    icesDataB <- RstoxData:::StoxAcoustic(list(RstoxData:::readXmlFile(paste0(exampleDir, "/", item), stream = T)))
